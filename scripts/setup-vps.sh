...
# ============================================
# Web2APK Gen 2 - VPS Setup Script
...
echo ""
echo -e "${CYAN}=========================================${NC}"
echo -e "${CYAN}  Web2APK Gen 2 - VPS Setup${NC}"
echo -e "${CYAN}=========================================${NC}"
echo ""

# Check if running as root or with 
if [ "$EUID" -ne 0 ]; then
    echo -e "${YELLOW}Note: Some commands may require  password${NC}"
fi
...
echo -e "${MAGENTA}[5/7] Setting up Android SDK...${NC}"
 apt install -y wget unzip zip lib32z1 lib32stdc++6
...
# Install required dependencies for Flutter
 apt install -y curl git xz-utils libglu1-mesa clang cmake ninja-build pkg-config libgtk-3-dev
...
# Clean old entries
 ...
 sed -i '/ANDROID_HOME/d' /etc/environment
 sed -i '/FLUTTER_HOME/d' /etc/environment

# Add new entries
echo "JAVA_HOME=$JAVA_HOME" |  tee -a /etc/environment
echo "ANDROID_HOME=$ANDROID_HOME" |  tee -a /etc/environment
echo "FLUTTER_HOME=$FLUTTER_HOME" |  tee -a /etc/environment
...
echo "  • Android SDK 34 + Build Tools"
echo "  • Flutter SDK"
echo "  • PM2"
echo ""
